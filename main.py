...
while True:
    ...
    if choice == "1":
        ...
        add_group(music_library, group, albums)

    elif choice == "2":
        group = input("Введіть назву групи для видалення: ")
        remove_group(music_library, group)

    elif choice == "3":
        group = input("Введіть назву групи для пошуку: ")
        search_result = search_group(music_library, group)
        ...
        edit_group(music_library, group, new_albums)

    elif choice == "5":
        filename = input("Введіть назву файлу для збереження: ")
        save_to_file(music_library, filename)

    elif choice == "6":
        filename = input("Введіть назву файлу для завантаження: ")
        music_library = load_from_file(filename)
